...
#' @param con A \code{\link[RSQLite:SQLiteConnection-class]{SQLiteConnection-class}} object, produced
#'   by \code{\link[DBI:dbConnect]{dbConnect()}} or \code{shinyNotes::connect_sqlite()}
#'   
...
#'   \code{\link[RSQLite:SQLiteConnection-class]{SQLiteConnection}} object for local development.
#'
...
#'
#' @return Returns an S4 object that inherits from \code{\link[DBI:DBIConnection-class]{DBIConnection}}. This object is
#'   used to communicate with the database engine. Under the hood,
#'   \code{dbConnect()} returns an object of class \code{\link[RSQLite:SQLiteConnection-class]{SQLiteConnection}}. See
#'   \code{\link[DBI:dbConnect]{dbConnect()}} for more details.
#'
#' @examples
#' connect_sqlite()
#'
...
#' @param error_value Error value to return if \code{\link[DBI:dbReadTable]{dbReadTable()}}
#'   fails. Default is \code{NA}.
#'
#' @return If the SQL query executes successfully, the return value will be an
#'   object of class \code{\link[tibble:tibble]{tibble}}. If an error is encountered,
